"""Microsoft Outlook (Mail, Calendar, Contacts) Data Connector Plugin for Gemini Enterprise.

Automates end-to-end setup for Microsoft Outlook in both:
- FEDERATED mode (real-time query + 21 BAP tool actions)
...
class OutlookPlugin(BaseConnectorPlugin):
  """Connector plugin for Microsoft Outlook."""

  @property
  def name(self) -> str:
    return "Microsoft Outlook (Mail, Calendar, Contacts)"
  ...
  def prompt_config_interactive(
    ...
    print("\n\033[1m\033[95m=== OUTLOOK CONNECTOR CONFIGURATION ===\033[0m")
    ...
    datastore_id = (
        input("Enter Data Store ID [default: outlook-ds]: ").strip()
        or "outlook-ds"
    )
  ...
  def execute_dry_run(self, config: Dict[str, Any]) -> str:
    ...
    enabled_actions = resolve_enabled_actions("outlook", access_level=access_level)

    plan_lines = [
        "\033[1m========================================================================\033[0m",
        "\033[1m                   OUTLOOK CONNECTOR DRY-RUN / CHANGE PLAN              \033[0m",
        "\033[1m========================================================================\033[0m",
        ...
        f"8. Discovery Engine Data Store: '{config.get('datastore_id', 'outlook-ds')}'",
  ...
  def provision(
    ...
    datastore_id = config.get("datastore_id", "outlook-ds")
    ...
    app_name = f"Gemini-Enterprise-Outlook-{datastore_id}"
    ...
    # ValidateSourceParametersByConnectorSource allowlist for 'outlook':
    # Top-level params accepts: {client_id, client_secret, instance_id}
    params = {
        "client_id": client_id,
    ...
    self.gcp_provider.create_discovery_engine_connector(
        ...
        collection_display_name=f"Microsoft Outlook ({tenant_id})",
  ...
  def verify(
    ...
    project_id = config["gcp_project"]
    location = config.get("location", "global")
    datastore_id = provision_result.get("datastore_id") or config.get("datastore_id", "outlook-ds")
  ...
  def generate_terraform(
    ...
    template_dir = os.path.join(base_dir, "terraform_templates", "outlook")
