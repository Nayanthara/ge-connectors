"""SharePoint Data Connector Plugin for Gemini Enterprise.

Automates end-to-end setup for Microsoft SharePoint Online in both:
- FEDERATED mode (real-time query + 18 BAP tool actions)
...
class SharePointPlugin(BaseConnectorPlugin):
  """Connector plugin for Microsoft SharePoint Online."""

  @property
  def name(self) -> str:
    return "Microsoft SharePoint Online"
  ...
  def prompt_config_interactive(
    ...
    print("\n\033[1m\033[95m=== SHAREPOINT CONNECTOR CONFIGURATION ===\033[0m")
    ...
    # 6. SharePoint Instance URL
    instance_uri = input(
        "Enter SharePoint Instance URL (REQUIRED, e.g. https://acme.sharepoint.com): "
    ).strip()
    while not instance_uri or not (
        instance_uri.startswith("http://") or instance_uri.startswith("https://")
    ):
      instance_uri = input(
          "SharePoint Instance URL cannot be empty. Please enter a valid URL starting with https://: "
      ).strip()
    ...
    # 10. Gemini Engine / App ID
    engine_id = input("Enter target Gemini Enterprise Engine/App ID to bind to (optional): ").strip()

    # 11. Data Store / Collection ID
    datastore_id = (
        input("Enter Data Store ID [default: sharepoint-ds]: ").strip()
        or "sharepoint-ds"
    )
  ...
  def validate_config(self, config: Dict[str, Any]) -> List[str]:
    ...
    if not instance_uri:
      errors.append("SharePoint Instance URL ('instance_uri') cannot be empty.")
    elif not (instance_uri.startswith("http://") or instance_uri.startswith("https://")):
      errors.append("Invalid 'instance_uri'. Must start with https:// (e.g. https://acme.sharepoint.com)")
  ...
  def execute_dry_run(self, config: Dict[str, Any]) -> str:
    ...
    enabled_actions = resolve_enabled_actions("sharepoint", access_level=access_level)

    plan_lines = [
        "\033[1m========================================================================\033[0m",
        "\033[1m                SHAREPOINT CONNECTOR DRY-RUN / CHANGE PLAN             \033[0m",
        "\033[1m========================================================================\033[0m",
        f"1. Connector Mode:             {mode}",
        f"2. Action Access Tier:         {access_level} ({len(enabled_actions)} BAP actions)",
        f"3. SharePoint URL:             {config.get('instance_uri')}",
        ...
        f"7. Secret Manager Storage:     '{config.get('datastore_id', 'sharepoint-ds')}-oauth-secret'",
        f"8. Discovery Engine Data Store: '{config.get('datastore_id', 'sharepoint-ds')}'",
  ...
  def provision(
    ...
    datastore_id = config.get("datastore_id", "sharepoint-ds")
    ...
    self.gcp_provider.create_discovery_engine_connector(
        ...
        collection_display_name=f"Microsoft SharePoint Online ({parsed_host})",
  ...
  def verify(
    ...
    project_id = config["gcp_project"]
    location = config.get("location", "global")
    datastore_id = provision_result.get("datastore_id") or config.get("datastore_id", "sharepoint-ds")
  ...
  def generate_terraform(
    ...
    template_dir = os.path.join(base_dir, "terraform_templates", "sharepoint")
